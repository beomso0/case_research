...
3. **Phase3 최종 docx** — 부합건 중 **유용성 방향성으로 정밀 재평가** → 상위 N을 `.claude/style_case_research.docx` 템플릿 표로. **유용성순 정렬(방향성 부합도→확정→심급), 판례번호 lbox 하이퍼링크, 사실관계·설시 bold.** → `output/{슬러그}_판례리서치_최종.docx`.
