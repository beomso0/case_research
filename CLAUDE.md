...
- `output/` — 작업 산출물(백업 docx·`_final_ranking.json` 등). **gitignore.**
- `.browser-profile/` — 브라우저 로그인 세션. **절대 커밋 금지**(gitignore).

**★최종 결과물(final docx) 저장 위치**: 환경변수 `RESEARCH_OUTPUT_DIR`가 설정돼 있으면 **그 경로**에 `{슬러그}_판례리서치_최종.docx`를 저장한다(미설정 시 repo `output/`). 이 머신에서는 `.claude/settings.local.json`(gitignore)의 `env`로 `C:\Users\univ3\iCloudDrive\Desktop\AI_\research_output`로 지정돼 있어 최종본이 그곳에 모인다. `build_docx.py`가 이 규칙을 구현한다. **주의: git 작업 저장소 자체는 iCloud 밖(`C:\Users\univ3\dev\claude_test`)에 두고, iCloud로는 최종 docx만 내보낸다**(과거 iCloud가 `.git`을 동기화하며 손상시킨 이력 — repo를 클라우드 동기화 폴더에 두지 말 것).
